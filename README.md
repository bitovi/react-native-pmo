...
While developing or making changes to the `.env` the emulator will used cached results, so you may not see your changes.
To amend this shut off the emulator and the use the `npm run clean` command. After starting up the emulator should take the new `.env` variables.

### Common Emulator Issues

When using the `npm run android` command, if an existing instance of the emulator is running it will add a message that says so. Be sure to make sure the terminal window that was hosting the React Native dev server isn't still on. 

When turning off the emulator, be sure to `Ctrl+C` to stop the server from running before closing the terminal window. If you're struggling to run the emulator because there is another instance running, there is a chance you may have closed the terminal before stopping the server. In this case, you may have to use Task Manager or Activity Monitor to close the rogue instance of the server. Finally, if none of the proposed solutions work, just trying restarting your work system.
